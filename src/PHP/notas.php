...
//Crea una nueva nota y la guarda en el archivo JSON.
 function createNota($texto, $importancia, $filePath) {
    ...
    $id = isset($notas[count($notas) - 1]['id']) ? $notas[count($notas) - 1]['id'] + 1 : 1;
...
// Elimina una nota por su ID.
 function deleteNotaById($id, $filePath) {
    ...
    // Guarda las notas actualizadas en el archivo JSON
    file_put_contents($filePath, json_encode(array_values($notasFiltradas), JSON_PRETTY_PRINT));
...
switch ($_SERVER['REQUEST_METHOD']) {
    case 'GET':
        if (isset($_GET['id'])) {
            ...
            echo json_encode($nota ?: ['error' => "No se encontró ninguna nota con ID $id."]);
        } else {
            $notas = getAllNotas($filePath);
    ...
    case 'POST':
        ...
        if (isset($input['texto']) && isset($input['importancia'])) {
            $nuevaNota = createNota($input['texto'], $input['importancia'], $filePath);
    ...
    case 'PUT':
        $input = json_decode(file_get_contents('php://input'), true);
        if (isset($input['id']) && isset($input['texto']) && isset($input['importancia'])) {
            $actualizada = updateNotaById($input['id'], $input['texto'], $input['importancia'], $filePath);
            echo json_encode($actualizada);
        } else {
            http_response_code(400); // Código de error: solicitud incorrecta
            echo json_encode(['error' => 'Datos incompletos para actualizar la nota.']);
        }
        break;
    ...
    case 'DELETE':
        ...
        if (isset($input['id'])) {
            ...
            http_response_code(400); // Código de error: solicitud incorrecta
            echo json_encode(['error' => 'ID de nota no proporcionado para eliminar.']);
        }
        break;
